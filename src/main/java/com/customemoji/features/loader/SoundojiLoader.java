...
import static com.customemoji.util.Result.Error;
import static com.customemoji.util.Result.Ok;
import static com.customemoji.util.Result.PartialOk;

import com.customemoji.util.Result;
...
public class SoundojiLoader
{
	...
	public void loadSoundojis()
	{
		this.soundojis.clear();

		if (SoundojiLoader.SOUNDOJIS_FOLDER.mkdir())
		{
			log.debug("Created soundoji folder: {}", SoundojiLoader.SOUNDOJIS_FOLDER);
			return;
		}

		Result<List<Soundoji>, List<Throwable>> result = this.loadSoundojisFromFolder(SoundojiLoader.SOUNDOJIS_FOLDER);
		result.ifOk(list ->
		{
